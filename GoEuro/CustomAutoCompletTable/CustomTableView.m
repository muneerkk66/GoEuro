...
- (BOOL) substringIsInDictionary:(NSString *)subString
{
    ...
    {
        range = ([[self.options valueForKey:CaseSensitive] isEqualToNumber:[NSNumber numberWithInt:1]]) ? [tmpString rangeOfString:subString] : [tmpString rangeOfString:subString options:NSCaseInsensitiveSearch];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.options valueForKey:UseSourceFont])
    {
        cell.textLabel.font = [self.options valueForKey:UseSourceFont];
    } else 
    {
